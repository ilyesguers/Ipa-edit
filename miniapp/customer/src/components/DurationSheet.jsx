...
export default function DurationSheet() {
  ...
  const banner = typeof selectedProduct.banner === 'string' && /^(https?:\/\/|\/)/.test(selectedProduct.banner) ? selectedProduct.banner : null;
  const close = () => { playSound('close'); useStore.setState({ showDurationSheet: false }); };
    ...
    <>
      ...
      <section className="fixed bottom-0 left-0 right-0 z-50 bottom-sheet rounded-t-[24px] max-h-[86dvh] overflow-y-auto" role="dialog" aria-modal="true" aria-label={localizedName(selectedProduct, locale)}>
        ...
        <div className="px-5 pb-6">
          ...
          <div className="flex items-start gap-3 pb-4 border-b border-[#2d3748] mt-1">
          ...
          <div className="section-heading mt-5"><PremiumIcon name="key" /><h3>{t(locale, 'chooseOption')}</h3></div>
...
function DurationRow({ duration, locale, onSelect }) {
  ...
  return (
    <button type="button" onClick={onSelect} disabled={!inStock} className={`duration-row w-full min-h-0 p-4 flex items-center justify-between gap-4 text-start rounded-2xl border ${inStock ? 'bg-[#161922] border-[#2d3748] hover:border-[#10b981]/50' : 'bg-[#11141a] border-[#1f2430] opacity-55'}`}>
      ...
      <span className="flex items-center gap-2 flex-none">
        ...
        {inStock ? <span className="w-9 h-9 rounded-xl bg-[#10b981] text-[#06110b] flex items-center justify-center"><PremiumIcon name={locale === 'ar' ? 'left' : 'right'} /></span> : <span className="text-[11px] text-[#9ca3af]">{t(locale, 'unavailable')}</span>}
      </span>
    </button>
  );
}
