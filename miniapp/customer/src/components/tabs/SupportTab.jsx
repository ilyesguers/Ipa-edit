...
export default function SupportTab() {
  ...
  return (
    <div className="p-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: -15, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="bg-gradient-to-br from-[#12121c] via-[#1a1a2e] to-[#12121c] rounded-[28px] p-6 border border-[#00ff88]/20 text-center relative overflow-hidden shadow-2xl">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(0,255,136,0.1),transparent_50%)]" />
        <motion.div animate={{ rotate: [0, 10, -10, 0], scale: [1, 1.1, 1] }} transition={{ duration: 3, repeat: Infinity }} className="text-[#00ff88] mb-4 relative inline-block">
          <PremiumIcon name="fire" size="3.5rem" />
          <motion.div animate={{ scale: [0, 1.5, 0], opacity: [0, 0.8, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 bg-[#00ff88]/20 rounded-full blur-xl" />
        </motion.div>
        <h2 className="text-[22px] font-black text-white relative font-[Orbitron]"><span className="gradient-text">{supportTitle}</span> 🔥</h2>
        <p className="text-[#8b8ba7] text-[13px] mt-2 leading-6 relative font-bold">{supportSubtitle}</p>
        <span className="inline-flex items-center gap-2 mt-4 text-xs text-[#00ff88] bg-[#00ff88]/10 border border-[#00ff88]/20 px-4 py-2 rounded-full font-black">
          <span className="w-2.5 h-2.5 rounded-full bg-[#00ff88] animate-pulse shadow-[0_0_10px_#00ff88]" /> {t(locale, 'availableNow')} ⚡
        </span>
      </motion.div>

      <div className="grid grid-cols-2 gap-3">
        <a href={`https://t.me/${supportUsername}`} target="_blank" rel="noreferrer" className="gamer-card rounded-[20px] p-5 flex flex-col items-center gap-3 text-center group hover:border-[#00ff88]/40">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#00ff88]/20 to-[#00d4ff]/15 flex items-center justify-center group-hover:scale-110 transition-transform">
            <PremiumIcon name="chat" size="2rem" className="text-[#00ff88]" />
          </div>
          <div>
            ...
            <p className="text-xs text-[#00ff88] font-bold mt-1">@{supportUsername}</p>
          </div>
          <span className="text-[10px] bg-[#00ff88]/10 text-[#00ff88] px-2.5 py-1 rounded-full border border-[#00ff88]/20">FAST ⚡</span>
        </a>
          ...
          <motion.div key={question} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }} className="gamer-card rounded-[18px] overflow-hidden">
            <button type="button" onClick={() => setOpenFaq(openFaq === index ? null : index)} className="w-full flex items-center justify-between p-4.5 text-right group">
              <span className="font-black text-white text-[13px] flex items-center gap-2">
                <span className="w-7 h-7 rounded-xl bg-[#00ff88]/10 border border-[#00ff88]/20 flex items-center justify-center text-[#00ff88] text-xs">{index + 1}</span>
                {t(locale, question)}
              </span>
              <motion.span animate={{ rotate: openFaq === index ? 180 : 0 }} className="text-[#00ff88]/60 group-hover:text-[#00ff88] transition-colors">
            ...
            <motion.div initial={false} animate={{ height: openFaq === index ? 'auto' : 0, opacity: openFaq === index ? 1 : 0 }} className="overflow-hidden">
              <p className="px-4.5 pb-4.5 text-[13px] text-[#8b8ba7] leading-7 bg-[#050508]/50 mx-3 mb-3 rounded-xl p-3 border border-[#1e1e32]">{t(locale, answer)}</p>
            </motion.div>
          </motion.div>
        ))}
      </div>
